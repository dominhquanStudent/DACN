...
function VoucherList() {
  ...
  const handleConfirmDelete = async () => {
    if (selectedVoucher) {
      try {
        await axios.delete(`/voucher/${selectedVoucher._id}`);
        const newVouchers = vouchers.filter(
          (voucher) => voucher._id !== selectedVoucher._id
        );
      ...
      <div className="flex w-full">
        ...
        <div className="w-3/4 border-l-2 border-gray-200 px-4">
              ...
              {Array.isArray(currentVouchers) &&
                currentVouchers.map((voucher: Voucher) => (
                  <tr key={voucher._id}>
                    ...
                    <td className="px-5 py-2 border-b border-gray-200 bg-white text-sm">
                      <button
                        onClick={() => handleDetailClick(voucher._id)}
          ...
          <ConfirmModal
            ...
            message={`Bạn có muốn xóa voucher ${selectedVoucher?.name} không?`}
          />
